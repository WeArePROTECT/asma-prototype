...
app = FastAPI(title="ASMA Demo API", version="0.3.1")
...
def preview_formulation(payload: FormPreviewIn):
    ...
    # Sum up complementarity and inhibition/competition among the chosen isolates
    for e in interactions:
        ...
        if a in chosen and b in chosen:
            ...
            if t == "complementarity":
                ...
                inh += s
            elif t == "competition":      # NEW: mild penalty
                inh += 0.5 * s

    # Base scoring: reward complementarity, penalize internal inhibition/competition
    score = 0.1 + 0.2 * comp - 0.3 * inh

    # Mild additional penalty for internal inhibitions only (average)
    inh_edges = [
